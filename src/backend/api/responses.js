...
export async function handleResponseRequest(request, env, ctx) {
  ...
  const matches = path.match(/\/api\/prompts\/([^\/]+)\/responses(?:\/([^\/]+))?/);
  ...
  const promptId = matches[1];
  const responseId = matches[2];
  ...
  if (!responseId && request.method === "POST") {
    ...
    if (!modelName || !content) {
      return new Response(
        JSON.stringify({ error: "Model name and response content are required" }),
        {
          status: 400,
          headers: { "Content-Type": "application/json" },
        }
